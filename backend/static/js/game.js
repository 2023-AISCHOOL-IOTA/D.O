$(function () {
  ...
  function startLineDrawing(node, color) {
    ...
    if (nodeInfo["change"]) {
      ...
      if (
        GLOBAL_FOOT_PRINT.hasOwnProperty(leftNode) &&
        GLOBAL_FOOT_PRINT.hasOwnProperty(rightNode)
      ) {
        var leftNodeInfo = GLOBAL_FOOT_PRINT[leftNode];
        var rightNodeInfo = GLOBAL_FOOT_PRINT[rightNode];
        if (
          leftNodeInfo["change"] &&
          leftNodeInfo["draw"] &&
          !!!GLOBAL_CHECK_FOOT_PRINT[leftNode] &&
  ...
  function userSetting() {
    ...
    for (var i = 0; i < userList.length; i++) {
      ...
      var left = x * 100 - 30;
      html +=
        '<div class="user-wrap" style="left:' +
        left +
        '"><input type="text" data-node="' +
        userList[i] +
        '"><button class="ladder-start" style="background-color:' +
        color +
        '" data-color="' +
        color +
        '" data-node="' +
  ...
  function resultSetting() {
    ...
    for (var i = 0; i < resultList.length; i++) {
      ...
      var left = x * 100 - 30;
      html +=
        '<div class="answer-wrap" style="left:' +
        left +
        '"><input type="text" data-node="' +
        node +
        '">';
  ...
  function drawDefaultLine() {
    ...
    for (var y = 0; y < heightNode - 1; y++) {
      ...
      for (var x = 0; x < widthNode - 1; x++) {
        html +=
          '<td style="width:98px; height:25px; border-left:2px solid #ddd; border-right:2px solid #ddd;"></td>';
      }
      html += "</tr>";
    }
    html += "</table>";
